...
void ShowVersion(int a_returnCode){
    cout<<endl;
    cout<<"   PET2Calibration version  0.2  "<<endl;
    cout<<endl;
    exit(a_returnCode);
}
...
void ShowHelp(int a_returnCode){
    ...
    cout<<endl;
    cout<<"***************************************************************************************************************"<<endl;
    cout<<" It's a application for Minfounders in MINFOUND-PET2L calibration. "<<endl;
    cout<<" Auther: Xiaozhuang Wang "<<endl;
    cout<<" Email: xiaozhuang.wang@minfound.com "<<endl;
    cout<<" Date: Dec. 28th, 2018 "<<endl; 
    cout<<endl;
    ...
    for(int i=1;i<argc;i++){
        ...
        if(option=="-h" || option=="--help" ||option=="-help") {
            ...
            if(argv[i+1]==NULL){
                ...
                string optionpara = (string) argv[i+1];
                FOV = int(acos( atoi(optionpara.c_str())/(2*R))*NCX/(2*PI) );
                if(FOV<0 ||FOV>=2*R){
                    cerr<<"******** invalid paramters for option:"<<option<<endl;
                    cerr<<"******** it should be  0~2R integral number     !!!  recommend among 300-700  ..."<<endl;
                    exit(1);
                }
                else{
                    cout<<"******** FOV cut  ---> "<< atoi(optionpara.c_str()) <<" valid cuts in transaxial pixels : "<<FOV<<endl;
                }
            } 
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
      ...
      const unsigned int ConincidenceFlag=0xFFFFFFFC;
	  ...
	  int tdiff,tdiff_cor1,tdiff_cor2;
      int multhit, ovflow;
      int Ax,Tx, Adc, coarse,fine,timezone;
      int Txmax,Txmin;
      double timetick;
      double coarseLSB=10.;       //ns;
      double fineLSB=0.0390625;      //ns;
...
////                                              histgream define                                               ////
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
cout<<"--------------Start histograms defination------"<<endl;
      ...
      TH1F *TimeRes         = hist1d("Time",2*HalfWindow,-HalfWindow,HalfWindow,"Time (ch)","Entries"); 
      ...
      TH1F *TimeRes_cor     = hist1d("Time_cor",2*HalfWindow,-HalfWindow,HalfWindow,"Time (ch)","Entries"); 
      ...
      TH2F *EnergyLutMap          = hist2d("EnergyLutMap",NCX,0,NCX,NZX,0,NZX,"TransAxial","Axial");
      TH2F *TimeLutMap          = hist2d("TimeLutMap",NCX,0,NCX,NZX,0,NZX,"TransAxial","Axial");
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
////                                              string for pdf/png                                              ////
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
     ////////////// string name for saving pdf/png;
cout<<"--------------Start path/pdf/png definination------"<<endl;
      string CalibResultsName        = path_to_output_file+"/"+"PET2L_Calibration_lut.root";
      string CalibHistName           = path_to_output_file+"/"+"PET2L_Calibration_histogram.root";
      string EnergyLUTFileName1      = path_to_output_file+"/"+"PET2L_EnergyLUT_Front.binary";
      string EnergyLUTFileName2      = path_to_output_file+"/"+"PET2L_EnergyLUT_Back.binary";
      string TimeLUTFileName1        = path_to_output_file+"/"+"PET2L_TimeLUT_Front.binary";
      string TimeLUTFileName2        = path_to_output_file+"/"+"PET2L_TimeLUT_Back.binary";
            ...
            while(fin.good()  && Readcounts<= ReadLimit ){
                ...
                if( CoincidenceFlag==datatest){ 
                    ...
                    for(int i=0; i<Nslotbyte; i++){
                        data[i] = PET3RawdataConvert(data[i]);
                    }
                    Readcounts +=Nslotbyte;
                    for(int i=0; i<Nbyte; i++){
    	                kind   = (data[2*i]   >>31 ) & (0x1  );
                        Adc2   = (data[2*i]        ) & (0x3FF);
                        Adc1   = (data[2*i]   >>10 ) & (0x3FF);
    	                tdiff  = (data[2*i]   >>20 ) & (0x7FF);
    	                Ax2    = (data[2*i+1]      ) & (0x3F );
    	                Tx2    = (data[2*i+1] >>6  ) & (0x3FF);
    	                Ax1    = (data[2*i+1] >>16 ) & (0x3F );
    	                Tx1    = (data[2*i+1] >>22 ) & (0x3FF);	 	
        ...
        if(data_type==single_data_type){
            while(fin.good() && Readcounts<= ReadLimit ){
                ...
                if( 0xFFFFFFFF != datatest){
                    ...
                    for(int i=0;i<Nbyte;i++){
                        if(( (data[2*i]>>31) & 0x1) == 0){
                            timezone = data[2*i]&0xFFFFFFFF ;
                        }
                        else{
                            Ax     = (data[2*i]     ) & 0x3F  ;
                            Tx     = (data[2*i]>>6  ) & 0x3FF ;
                            Adc    = (data[2*i]>>16 ) & 0x3FF ;
                            fine   = (data[2*i+1]   ) & 0xFF  ;
                            coarse = (data[2*i+1]>>8) & 0x7FF ;
                            timetick = coarse*coarseLSB+fine*fineLSB;  
                           
                            index = Tx*NZX+Ax;
                            if(EnergyCalibOn)Energy[index] ->Fill(Adc);
                        }
                    }
                }
            }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for(int i=0;i<NPixels;i++){
        if(TimeCalibOn){
            ...
            tmean[i] =tmean[i]+ Time[i]->GetMean() ;
	}
    }
...
cout<<"--------------Start Calibration------"<<endl;
    ...
    if(EnergyCalibOn){
	    ...
	    fitfun[i]->SetParameters(Energy[i]->GetBinContent(xpos[npeak-1]), peak0[i], 25.);
            Energy[i]->Fit(fitfun[i],"QNC","",peak0[i]-30,peak0[i]+30 );
    ...
    if(TimeCalibOn){
                    ...
                    while(fin.good()  && Readcounts<= ReadLimit ){
                        ...
                        if( CoincidenceFlag==datatest){ 
                            ...
                            for(int i=0; i<Nbyte; i++){
                                ...
                                tdiff_cor1 =  tdiff -tmean[Tx1*NZX+Ax1];
                                tdiff_cor2 = -tdiff -tmean[Tx2*NZX+Ax2];
                                if(tdiff>=HalfWindow)continue;   // time is too large;
        ...
        for(int i=0;i<NPixels;i++){
            ...
            fitfun[i]->SetParameters(coeff0,coeff1,coeff2);
            //Time[i]->Fit(fitfun[i],"QNC","",coeff1-5*coeff2,coeff1+5*coeff2);
            Time[i]->GetXaxis()->SetRangeUser(coeff1-200,coeff1+200);
...
cout<<"--------------Start saving----"<<endl;
	...
	FILE *energylut1,*energylut2, *timelut1, *timelut2;
	energylut1=fopen(EnergyLUTFileName1.c_str(),"wb");
	energylut2=fopen(EnergyLUTFileName2.c_str(),"wb");
	timelut1  =fopen(TimeLUTFileName1.c_str(),"wb");
	timelut2  =fopen(TimeLUTFileName2.c_str(),"wb");
   	    ...
   	    for(int j=0;j<NZX;j++){
   	    	for(int k=0;k<TxInBank;k++){
   	    	    ...
   	    	    par2=0;
                    ...
                    tpar=0;
////    ///////////////////////////////////////////////////////////////////////////////////////////////////
                   if( peakt0>=1. && peakt1<1.*Nbin && sigmat>=1. && sigmat<1.*Nbin  ){
       ...
       ////////////////////////////////////// //////////////////////////////////////////////////////////////                
   	    	       if(EnergyCalibOn){
                       fwrite(&aa,sizeof(unsigned int),1,energylut1);
                       EnergyLutMap->SetBinContent(Tx+1,Ax+1,gain);
                   }
                   tpar= static_cast<short>(tdelay);
                   if(TimeCalibOn){
                       fwrite(&tpar,sizeof(short),1,timelut1);
...
////    ///////////////////////////////////////////////////////////////////////////////////////////////////
       ...
       ////////////////////////////////////// //////////////////////////////////////////////////////////////                
   	    	       if(EnergyCalibOn){
                       fwrite(&aa,sizeof(unsigned int),1,energylut2);
                       EnergyLutMap->SetBinContent(Tx+1,Ax+1,gain);
                   }
                   tpar= static_cast<short>(tdelay);
                   if(TimeCalibOn){
                       fwrite(&tpar,sizeof(short),1,timelut2);
                       TimeLutMap->SetBinContent(Tx+1,Ax+1,tdelay);
                   }
   	    		//counter+i+;
                   tr->Fill();
...
////                }
////                tv[i] = c2->cd(i%16+1);
////                Time[i]->GetXaxis()->SetRangeUser(-30./fineLSB, 30./fineLSB);
////                Time[i]->Draw();
////            
////                if(i%16==15){
...
////        ifstream energyfileread;
////        ifstream timefileread;
////        
////        energyfileread.open(EnergyLUTFileName1.c_str(),ios::binary);
////        timefileread.open(TimeLUTFileName1.c_str(),ios::binary);
////        for(int i=0;i<halfNmod;i++){ 
////   	        for(int j=0;j<NZX;j++){
////   	        	for(int k=0;k<TxInBank;k++){
////                
////                    energyfileread>>aa;
////                    timefileread>>tpar;
////       			    if(EnergyCalibOn)energytxtfile<<"Ax "<<j<<" Tx "<<i*TxInBank+k<<"  aa "<<(aa&0xFFFF)<<"  bb"<<((aa>>16)&0xFFFF)<<endl;;
////                    if(TimeCalibOn)timetxtfile<<"Ax "<<j<<" Tx "<<i*TxInBank+k<<"  tdelay "<<tpar<<endl;
////                } 
////            }
////        }  
////
////        energyfileread.open(EnergyLUTFileName2.c_str(),ios::binary);
////        timefileread.open(TimeLUTFileName2.c_str(),ios::binary);
////        for(int i=0;i<halfNmod;i++){ 
////   	        for(int j=0;j<NZX;j++){
////   	        	for(int k=0;k<TxInBank;k++){
////                
