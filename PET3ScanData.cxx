...
*/
    ...
    ofstream txtout;
    string txtfilestr = path_to_output_file +"/"+"out.txt";
    txtout.open(txtfilestr.c_str());
        ...
        if(data_type==coin_data_type){
           ...
           int flagx=0;
            while(fin.good() &&Readcounts<= ReadDepth ){
                ...
                idx++;
                txtout<<dec<<idx<<"   "<<hex<<datatest<<endl;
                Readcounts +=1;
                //datatest = PET3RawdataConvert(datatest);
                if(datatest==0xFFFFFFFC){
                    if(idx-flagx!=65)cout<<dec<<idx-flagx<<"   "<<hex<<datatest<<endl;
                //    cout<<dec<<idx -flagx<<"   "<<hex<<datatest<<endl;
                    flagx = idx ;
