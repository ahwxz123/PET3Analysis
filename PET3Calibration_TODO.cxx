...
const unsigned int TLUTN=58368000;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      ...
      for(int i=0; i<NPixels; i++){
          ...
          if(EnergyCalibOn){
              ...
              Energy[i] = hist1d(buf,Nbin,0,Nbin,"Energy","Entries");
          }
          if(TimeCalibOn){
              sprintf(buf,"Time%d", i);
              Time[i] = hist1d(buf,2*HalfWindow,-HalfWindow,HalfWindow,"Time","Entries");
          }   
      }  
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
     ////////////// string name for saving pdf/png;
      string CalibResltsName         = path_to_output_file+"/"+"PET3Calibration_lut.root";
      string CalibHistName           = path_to_output_file+"/"+"PET3Calibration_histogram.root";
      string EnergyLUTFileName       = path_to_output_file+"/"+"PET3EnergyLUT.raw";
      string TimeLUTFileName         = path_to_output_file+"/"+"PET3TimeLUT.raw";
      ...
      string EnergySpectrumPdf       = EnergySpectrumName+".pdf";
      string TimeSpectrumPdf         = TimeSpectrumName+".pdf";
      
      string EnergySpectrumPng       = EnergySpectrumName+".pdf";
      string TimeSpectrumPng         = TimeSpectrumName+".pdf";
      
      string EnergySpectrumBin       = EnergySpectrumName+".binary";
      string TimeSpectrumBin         = TimeSpectrumName+".biinary";
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if(TimeCalibOn){
        for(int i=0;i<NPixels;i++){
            ...
            fitfun[i]->SetParameters(Time[i]->GetMaximum(), tx,50);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//int temp1;
	FILE *fileout;
	fileout=fopen(EnergyLUTFileName.c_str(),"wb");

	unsigned int startflag=0;
	fwrite(&startflag,sizeof(int),1,fileout);
	fwrite(&startflag,sizeof(int),1,fileout);
	fwrite(&startflag,sizeof(int),1,fileout);
	startflag = 0xFFFF;
	startflag = PET3RawdataConvert(startflag);
	fwrite(&startflag,sizeof(int),1,fileout);

	unsigned int zeros=0;
	ofstream filetxt;
	sprintf(buf,"%s/LUTfile_%d.txt",path_to_output_file.c_str(),sysdate);
	filetxt.open(buf);
	int counter= 0;
	filetxt<<"   index"<<counter<<"   value  "<<0xFFFF0000<<"   aa  "<<0xFFFF<<"   bb   "<<0<<endl;
	counter++;
	filetxt<<"   index"<<counter<<"   value  "<<0<<"   aa  "<<0<<"   bb   "<<0<<endl;
	counter++;
	filetxt<<"   index"<<counter<<"   value  "<<0<<"   aa  "<<0<<"   bb   "<<0<<endl;
	counter++;				
	filetxt<<"   index"<<counter<<"   value  "<<0<<"   aa  "<<0<<"   bb   "<<0<<endl;
	...
	for(int i=0;i<76;i++){
		for(int j=0;j<8;j++){
			for(int k=0;k<8;k++){
				...
				fwrite(&zeros,sizeof(int),1,fileout);
...
/////////////////////////////////////////// twp peaks calibration//////////////////////////////////////////////                
//                if(peakV[j][temp1]>0 && peak[j][temp1]>peakV[j][temp1]){
//					aa = static_cast<unsigned short>(  (1275.-511.)/(peak[j][temp1]-peakV[j][temp1]) *8192);
//				    bb = static_cast<short>(  (511.*peak[j][temp1] - 1275.*peakV[j][temp1])/(peak[j][temp1]-peakV[j][temp1]));	    
//				}
//				if(peakV[j][temp1]>0 && peak[j][temp1]==0){
//					aa = static_cast<unsigned short>(  (511.)/(peakV[j][temp1]) *8192);
//				}
/////////////////////////////////////////// //////////////////////////////////////////////////////////////                
////////////////////////////////////////    one peak calibration    //////////////////////////////////////                
				if(peak0[j+temp1*NZX]>0 ){
					aa = static_cast<unsigned short>(  (511.)/(peak0[j+temp1*NZX]) *8192);
				}
/////////////////////////////////////////// //////////////////////////////////////////////////////////////                
				...
				fwrite(&wxz,sizeof(unsigned int),1,fileout);
...
//.............................................................................................................//
	...
	sprintf(buf,"%s/Results_LUT_%d_fit.root",path_to_out_file.c_str(),sysdate);  
    TFile *rf=new TFile(buf,"recreate");
    TTree *tr=new TTree("npeak","npeak");
 ...
 ///////////////////////////////////////////////////////// two peaks calibration;

//			if(peakV[i][j]>0 && peak[i][j] > peakV[i][j]){
//				par1 = static_cast<unsigned short>(  (1275.-511.)/(peak[i][j]-peakV[i][j]) *8192);
//			    par2 = static_cast<short>(  (511.*peak[i][j] - 1275.*peakV[i][j])/(peak[i][j]-peakV[i][j]));
//				gain = (1275.-511.)/(peak[i][j]-peakV[i][j]);
//				shift = (511.*peak[i][j] - 1275.*peakV[i][j])/(peak[i][j]-peakV[i][j]);
//			}
//			if(peakV[i][j]>0 && peak[i][j]==0){
//				par1 = static_cast<unsigned short>(  (511.)/(peakV[i][j]) *8192);
//			    par2 = static_cast<short>(0) ;
//				gain = (511.)/(peakV[i][j]);
//				shift = 0;
//			}                  // two peaks calibration;
 ////////////////////////////////////////////////////////////////////////////////////////////////////////////////// 
