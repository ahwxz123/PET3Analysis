...
void ShowVersion(int a_returnCode){
    cout<<endl;
    cout<<"   PET3Calibration version  0.4  "<<endl;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      ...
      TH1F *TimeRes = hist1d("Timeresolution",2*HalfWindow,-HalfWindow,HalfWindow,"Time (ch)","Entries"); 
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if(TimeCalibOn){
                ...
                if(data_type==coin_data_type){
                    while(fin.good()  && Readcounts<= ReadLimit ){
                        ...
                        for(int i=0; i<Nbyte; i++){
                            ...
                            tdiff_cor1 = TMath::Floor( tdiff -tmean[Tx1*NZX+Ax1]);
                            tdiff_cor2 = TMath::Ceil( -tdiff -tmean[Tx2*NZX+Ax2]);
                            ...
                            if(TimeCalibOn){
                                index1 = Tx1*NZX*2*HalfWindow+Ax1*2*HalfWindow+tdiff_cor1+HalfWindow;
                                Tdata[index1] ++;
                                index2 = Tx2*NZX*2*HalfWindow+Ax2*2*HalfWindow+tdiff_cor2+HalfWindow;
                                Tdata[index2] ++;
                           }

                           if(NN==Niter-1){
                                TimevsAngle->Fill(Tx1,tdiff_cor1);
                                TimevsAngle->Fill(Tx2,tdiff_cor2);
                                TimevsRing ->Fill(Ax1,tdiff_cor1);
                                TimevsRing ->Fill(Ax2,tdiff_cor2);
                           
                                TimeRes ->Fill(tdiff_cor1);
                                TimeRes ->Fill(tdiff_cor2);
                           }
                        }
        ...
        for(int i=0;i<NPixels;i++){
            for(int j=0;j<2*HalfWindow;j++){
                if( abs(j-HalfWindow)<=100){
                    Time[i]->SetBinContent(j+1,Tdata[i*2*HalfWindow+j]);
                }
                else{
                    Time[i]->SetBinContent(j+1,0);
      ...
      cout<<"------ time Calibration iteration "<< NN<<" has been done ..."<<endl;
      }
