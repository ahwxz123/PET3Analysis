...
void ShowVersion(int a_returnCode){
    cout<<endl;
    cout<<"   PET3Calibration version  1.3  "<<endl;
...
//    cout<<"     :Cocincidence flag 0xFFFFFFFC; 16vevents in one flag;       2019/01/10 "<<endl;
    ...
    cout<<"     :Time lut format   XXXXXXXXXXXXXXXX XXXXXXXX000XXXXX .      2019/01/14 "<<endl;
    cout<<"     :      High 32 all zeros |coarse(9bit)|fix(3bit)|fine(5bit) 2019/01/14 "<<endl;
    cout<<"     :Add Submodule Energy;                                      2019/01/14 "<<endl;
    cout<<"     :Singles  High32 Low 32 exchange.                           2019/01/18 "<<endl;
    cout<<"     :SetStats flag On, 2D floop map flag-off                    2019/01/21 "<<endl;
    cout<<"     : -tmean[index1]+tmean[index2];                             2019/01/21 "<<endl;
    cout<<"     : read all data in files      ;                             2019/01/21 "<<endl;
    cout<<"     : Only Time calibration       ;                             2019/01/21 "<<endl;
...
{
std::cout<<"*********************** Only Time Calibration *************************"<<endl;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
	  ...
	  const unsigned int TimeLutFiller=0xA0A;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      ...
      for(int i=0; i<NPixels; i++){
        ...
        //  }
          if(TimeCalibOn){
              ...
              sprintf(buf,"TimeBK_Ax%d_Tx%d", i%NZX,i/NZX);
              TimeBK[i] = hist1dS(buf,2*HalfWindow,-HalfWindow,HalfWindow,"Time (ch)","Entries");
          }   
      }  
      ...
      TH2F *EnergyvsAngle = hist2d("EnergyvsAngle",NCX,0,NCX,Nbin,0,Nbin,"Angle (0-607) ","Energy (ch)");
      TH2F *EnergyvsRing  = hist2d("EnergyvsRing" ,NZX,0,NZX,Nbin,0,Nbin,"Ring (0-47)","Energy (ch)");
      TH2F *EnergyPeakMap   = hist2d("EnergyPeakMap" ,NCX,0,NCX,NZX,0,NZX," Transxial (0-47) ","Axial (0-47)");
      TH2F *EnergyLutMap    = hist2d("EnergyLutMap" ,NCX,0,NCX,NZX,0,NZX," Transxial (0-47) ","Axial (0-47)");

      TH1F *TimeRes     = hist1d("Timeres",2*HalfWindow,-HalfWindow,HalfWindow,"Time diff(0.3125ps)","Entries"); 
      TH2F *TimevsAngle = hist2d("TimevsAngle",NCX,0,NCX,2*HalfWindow,-HalfWindow,HalfWindow," Angle ","Time diff(0.3125ps)");
      TH2F *TimevsRing  = hist2d("TimevsRing" ,NZX,0,NZX,2*HalfWindow,-HalfWindow,HalfWindow," Ring ","Time diff(0.3125ps)");
      TH1F *TimeResBK     = hist1d("Timeres_BK",2*HalfWindow,-HalfWindow,HalfWindow,"Time diff(0.3125ps)","Entries"); 
      TH2F *TimevsAngleBK = hist2d("TimevsAngle_BK",NCX,0,NCX,2*HalfWindow,-HalfWindow,HalfWindow," Angle ","Time diff(0.3125ps)");
      TH2F *TimevsRingBK  = hist2d("TimevsRing_BK" ,NZX,0,NZX,2*HalfWindow,-HalfWindow,HalfWindow," Ring ","Time diff(0.3125ps)");

      TH1F *TimeRes_cor     = hist1d("Timeres_cor",2*HalfWindow,-HalfWindow,HalfWindow,"Time diff(0.3125ps)","Entries"); 
      TH2F *TimevsAngle_cor = hist2d("TimevsAngle_cor",NCX,0,NCX,2*HalfWindow,-HalfWindow,HalfWindow," Angle (0-607) ","Time diff(0.3125ps)");
      TH2F *TimevsRing_cor  = hist2d("TimevsRing_cor" ,NZX,0,NZX,2*HalfWindow,-HalfWindow,HalfWindow," Ring (0-47) ","Time diff(0.3125ps)");
      TH2F *TimeLutMap      = hist2d("TimeLutMap" ,NCX,0,NCX,NZX,0,NZX," Transxial (0-607) ","Axial (0-47)");
      ...
      TimevsAngle_cor->SetStats(kFALSE);
      TimevsRing_cor ->SetStats(kFALSE);
      EnergyPeakMap  ->SetStats(kFALSE);
      EnergyLutMap   ->SetStats(kFALSE);
      TimeLutMap     ->SetStats(kFALSE);

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
////                                              string for pdf/png                                              ////
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
     ////////////// string name for saving pdf/png;
      string CalibResultsName        = path_to_output_file+"/"+"Calibration_LUT_Iteration"+to_string(Niter)+".root";
      string CalibHistName           = path_to_output_file+"/"+"Calibration_Histogram_Iteration"+to_string(Niter)+".root";
            ...
            while(fin.good()   ){
                ...
                if(CoincidenceFlag!= datatest){
                    ...
                    for(int i=0; i<Nbyte; i++){
                       ...
                       if(kind==0){
                         if(TimeCalibOn){
                             TimeBK[index1]->Fill( -tdiff); 
                             TimeBK[index2]->Fill(  tdiff); 

                             TimevsAngleBK->Fill(Tx1,-tdiff);
                             TimevsAngleBK->Fill(Tx2, tdiff);
                             TimevsRingBK ->Fill(Ax1,-tdiff);
                             TimevsRingBK ->Fill(Ax2, tdiff);
                             
                             TimeResBK ->Fill( tdiff);
                             TimeResBK ->Fill(-tdiff);
                        }
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    for(int i=0;i<NPixels;i++){
        if(TimeCalibOn){
            for(int k=0;k<2*HalfWindow;k++){
                count1 = Time[k]->GetBinContent(k+1);
                count2 = TimeBK[k]->GetBinContent(k+1);
                Time[k]->SetBinContent(k+1,count1-count2); 
            }
            ...
            Time[i]->Fit(fitfun[i],"QNC","",coeff1-coeff2,coeff1+coeff2);
...
/*    if(EnergyCalibOn){
  ...
  */  
                        ...
                        if(CoincidenceFlag!=datatest){
                            ...
                            for(int i=0; i<Nbyte; i++){
                                ...
                                index2 = Tx2*NZX +Ax2;
                                     ...
                                     tdiff_cor1 = static_cast<double>(-tdiff) -tmean[index1] +tmean[index2];
                                     tdiff_cor2 = static_cast<double>( tdiff) -tmean[index2] +tmean[index1];
                                    

                                     
                                     //if(Adc1<=peak0[index1]-1.5*res[index1]||Adc1>=peak0[index1]+1.5*res[index1])continue;
                                     //if(Adc2<=peak0[index2]-1.5*res[index2]||Adc2>=peak0[index2]+1.5*res[index2])continue;
                                ...
                                if(kind==0){
                                     TimeBK[index1]->Fill( tdiff_cor1);
                                     TimeBK[index2]->Fill( tdiff_cor2);
                                     if(NN==Niter-1){
                                         TimevsAngleBK->Fill(Tx1,tdiff_cor1);
                                         TimevsAngleBK->Fill(Tx2,tdiff_cor2);
                                         TimevsRingBK ->Fill(Ax1,tdiff_cor1);
                                         TimevsRingBK ->Fill(Ax2,tdiff_cor2);
                                         TimeResBK ->Fill(tdiff_cor1);
                                         TimeResBK ->Fill(tdiff_cor2);
                                     }
    ...
    cout<<"------ time Calibration iteration "<< NN+1<<" has been done ..."<<endl;
     ...
     TimeRes ->Fit(fitf1,"QNC");
     TimeRes_cor ->Fit(fitf2,"QNC");
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    const unsigned int HeaderFlag=0x0;
    double gain,shift;
    double tdelay;
    unsigned short par1;
    short par2,tpar;
   ...
   for(int i=0;i<Nbank;i++){
   	...
   	for(int j=0;j<NZX;j++){
   		for(int k=0;k<TxInSubmod;k++){
            ...
            tpar=HeaderFlag;
///////////////////////////////////////////////////////////////////////////////////////////////////////
               if( peakt0>=1. && peakt0<1024. && sigmat>=1 && sigmat<1024.  ){
   ...
   /////////////////////////////////////// //////////////////////////////////////////////////////////////                
        ...
        //        fwrite(&wxz,sizeof(unsigned int),1,energylut);
   		...
   		//        EnergyPeakMap ->SetBinContent(Tx+1,Ax+1,peakt0);	
        //    }
            
            
            tpar= static_cast<short>(tdelay);
            short tpartemp = ((tpar<<8)&0xFF00)+(tpar&0x1F);
...
/*
    ...
    if(Save_Png){
        ...
        if(TimeCalibOn){
            ...
            ttxt1.SetNDC();
            ttxt2.SetNDC();
            float mean1 =fitf1->GetParameter(1);
            float sig1  =fitf1->GetParameter(2);
            float mean2 =fitf2->GetParameter(1);
            float sig2  =fitf2->GetParameter(2);
            ttxt1.SetTextSize(0.06);
            ttxt2.SetTextSize(0.06);
            ttxt1.SetTextColor(1);
            ttxt2.SetTextColor(4);
            sprintf(buf, "Mean :%3.2f  sigma:%3.2f",mean1,sig1);
            sprintf(buf2,"Mean :%3.2f  sigma:%3.2f",mean2,sig2);
            ttxt1.SetText(0.27,0.3,buf);
            ttxt2.SetText(0.17,0.3,buf2);
...
cout<<"-----------------energy spectrum has been done----------------------------"<<endl;
